...
ブラウザー版 UI は静的な GitHub Pages サイトとして動作します。

```text
https://semanticwave-hoyeon.github.io/paper-palette/
```

GitHub Pages は HTML、CSS、JavaScript などの静的ファイルをホストします。
そのため、Web UI は Python サーバーではなくブラウザー内でパレットを生成します。
パレット生成、プリセット適用、色のロック、ブラウザーの色選択 UI での編集、
Python 配列文字列のコピー、PNG 保存に対応しています。
